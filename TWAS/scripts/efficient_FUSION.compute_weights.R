...
## Updated by Fahri Kucukali based on threading suggestions discussed here: https://github.com/gusevlab/fusion_twas/issues/16  
#options(echo = TRUE)
...
option_list = list(
  ...
  make_option("--threads", action="store", default=1,
...
weights.bslmm = function( input , bv_type , snp , out=NA ) {
	...
	arg = paste( opt$PATH_gemma , " -outdir . -miss 1 -maf 0 -r2 1 -rpace 1000 -wpace 1000 -bfile " , input , " -bslmm " , bv_type , " -o " , out , sep='' )
...
arg = paste( opt$PATH_plink ," --allow-no-sex --bfile ",opt$bfile," --pheno ",pheno.file," --keep ",pheno.file," --make-bed --out ",geno.file, " --threads ", opt$threads, sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
# 1. generate GRM
arg = paste( opt$PATH_plink," --allow-no-sex --bfile ",opt$tmp," --make-grm-bin --out ",opt$tmp, " --threads ", opt$threads, sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
for ( i in 1:opt$crossval ) {
	...
	arg = paste( opt$PATH_plink ," --allow-no-sex --bfile ",opt$tmp," --keep ",cv.file,".keep --out ",cv.file," --make-bed", " --threads ", opt$threads, sep='')
	system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
